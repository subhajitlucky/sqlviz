  ...
  return (
    <div className="w-full h-64 bg-slate-950 rounded-xl border border-slate-800 relative overflow-hidden flex flex-col items-center justify-center p-6">
      <div className="grid grid-cols-4 gap-3">
        {pages.map((page, i) => (
          <div key={i} className="relative w-12 h-10 rounded border border-slate-800 bg-slate-900 overflow-hidden">
            {/* Page content lines */}
            <div className="p-1 space-y-1">
              <div className="h-0.5 w-full bg-slate-800 rounded"></div>
              <div className="h-0.5 w-3/4 bg-slate-800 rounded"></div>
              <div className="h-0.5 w-full bg-slate-800 rounded"></div>
            </div>
      ...
      {/* Progress Bar */}
      <div className="w-full mt-6 h-1 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
        <motion.div
